...
export class EventsBridge implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private handleEventStoreChange(change: ChangeStreamInsertDocument<EventDocument>) {
    ...
    const eventInstance = this.eventDeserializer.deserialize<typeof insertedEvent>(insertedEvent);
